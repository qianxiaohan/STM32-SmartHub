...
/**
 * @brief       保存校准参数
 *   @note      参数保存在FLASH芯片里面(W25Q64),起始地址为TP_SAVE_ADDR_BASE.
 *              占用大小为13字节
 * @param       无
 * @retval      无
 */
void w25qxx_save_adjust_data(void)
{
    uint8_t *p = (uint8_t *)&tp_dev.xfac;   /* 指向首地址 */
    uint8_t flag = 0X0A;
    /* p指向tp_dev.xfac的地址, p+4则是tp_dev.yfac的地址
    ...
    // at24cxx_write(TP_SAVE_ADDR_BASE, p, 12);                /* 保存12个字节数据(xfac,yfac,xc,yc) */
    // at24cxx_write_one_byte(TP_SAVE_ADDR_BASE + 12, 0X0A);   /* 保存校准值 */
    norflash_write(p, TP_SAVE_ADDR_BASE, 12);   /* 保存12个字节数据(xfac,yfac,xc,yc) */
    norflash_write(&flag, TP_SAVE_ADDR_BASE + 12, 1); /* 保存校准值 */
}

/**
 * @brief       获取保存在FLASH里面的校准值
 * @param       无
 * @retval      0，获取失败，要重新校准
 *              1，成功获取数据
 */
uint8_t w25qxx_get_adjust_data(void)
{
    uint8_t *p = (uint8_t *)&tp_dev.xfac;
    uint8_t temp = 0;
     ...
     * 据类型. 此方法适用于各种数据(包括结构体)的保存/读取(包括结构体).
     */
    norflash_read(p, TP_SAVE_ADDR_BASE, 12);    //读取xfc, yfc, xoff, yoff共12个字节
    norflash_read(&temp, TP_SAVE_ADDR_BASE + 12, 1);  //读取校正标志位
